...
class DeepMusicVisualizer(object):
	def __init__(self, 
		song,
		resolution = '512',
		duration=False,
		...
		self.tempo_sensitivity=tempo_sensitivity
		self.depth=depth
		self.classes=classes
		self.sort_classes_by_power=sort_classes_by_power
		self.jitter=jitter
		self.frame_length=frame_length
		self.truncation=truncation
		self.smooth_factor=smooth_factor
		self.batch_size=batch_size
		self.use_previous_classes=0
		self.use_previous_vectors=0
	...
	def generate(self, output_file):
		...
		model_name='biggan-deep-' + self.resolution

		frame_length=self.frame_length

		#set pitch sensitivity
		pitch_sensitivity=(300-self.pitch_sensitivity) * 512 / frame_length

		#set tempo sensitivity
		tempo_sensitivity=self.tempo_sensitivity * frame_length / 512
		...
		sort_classes_by_power=self.sort_classes_by_power
		...
		use_previous_vectors=self.use_previous_vectors

		#set use_previous_vectors
		use_previous_classes=self.use_previous_classes
		...
		if self.smooth_factor > 1:
			smooth_factor=int(self.smooth_factor * 512 / frame_length)
		else:
		...
		elif self.use_previous_classes==1:
